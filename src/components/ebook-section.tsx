...
export default function EbookSection({ product }: EbookSectionProps) {
    if (!product) {
        // Fallback or return null if no featured product is found
        return null;
    }
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-8">
                <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
                    ...
                    >
                            ...
                            {product.features?.slice(0, 3).map((feature, index) => (
                                <div key={index}>
                                    <div className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                                        <div className="w-1.5 h-1.5 rounded-full bg-[#CE1117]" />
                                        {feature}
                                    </div>
                                </div>
                            ))}
                        ...
                        >
                            <Link href={`/products/${product.slug.current}`} className="group inline-flex items-center justify-center gap-3 text-white px-8 py-3 rounded-full font-medium transition-all cursor-pointer bg-[#CE1117]">
                                Buy Now
                                <svg
                            ...
                            <Link href="/products" className="inline-flex items-center justify-center gap-2 px-8 py-3 rounded-full font-medium text-amber-700 dark:text-amber-300 border-2 border-[#CE1117] dark:border-amber-600/50 hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-all cursor-pointer">
                                View All Products
                            </Link>
                        </motion.div>
                    </motion.div>
